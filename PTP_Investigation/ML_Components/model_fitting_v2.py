...
def Plot_With_Training (pr_metrics, pr_saveas, roc_metrics, roc_auc, roc_saveas, metric_saveas, tr_metrics, tr_roc, tr_roc_auc):
    pr_metrics = pr_metrics.drop(columns=['Sensitivity'])
    # Plot scores from metrics method as pr and roc curves
    plt.rcParams["font.family"] = "arial"   # define our font (arial carried through figs)
    ...
    plt.clf()
    plt.plot(roc_metrics['fpr'],roc_metrics['tpr'],label="Model Testing, ROC AUC="+str(round(roc_auc, 3)))
    plt.plot(tr_roc['fpr'], tr_roc['tpr'], label="Model Training, ROC_AUC="+str(round(tr_roc_auc, 3)))
    plt.legend(title='', bbox_to_anchor=(.5,1), loc='lower center', ncol=2)
    plt.ylim(0,1)
    plt.xlim(0,1)
    ...
    tr_pr_auc_score = auc(tr_metrics['Recall'], tr_metrics['Precision'])
    plt.plot(pr_metrics['Recall'],pr_metrics['Precision'], label='Model Testing (PR AUC = '+str(round(pr_auc_score, 3))+')')
    plt.plot(tr_metrics['Recall'], tr_metrics['Precision'], label='Model Training (PR AUC = '+str(round(tr_pr_auc_score, 3)) + ')')
    plt.axline((0, 0), slope=0, color='gray', linestyle='dashed')
    plt.legend(title='', bbox_to_anchor=(.5,1), loc='lower center', ncol=2)
